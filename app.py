...
def geocode_address_resilient(address_str: str) -> Tuple[Optional[float], Optional[float], str]:
    ...
    Cached by address string so re-clicking / reruns with the same address
    don't re-hit three external APIs every time.
    """
...
def compress_image_bytes(file_bytes: bytes, max_width: int = 1200, max_height: int = 900, quality: int = 75) -> bytes:
    """Compress and convert image to JPEG.

    IMPORTANT: this is cached (keyed on the raw file bytes) because Streamlit
    reruns the entire script on every widget interaction (typing, clicking,
    expanding a section, etc). Without caching, every uploaded photo was being
    re-decoded/re-resized/re-encoded from scratch on *every single rerun*,
    which is what made the app appear to freeze/spin once a field rep had
    uploaded more than a handful of photos. With caching, a given photo is
    only ever compressed once per session.
    """
...
def process_uploaded_photos(uploaded_files: List) -> List[Dict]:
    """Process and compress uploaded photos (compression itself is cached
    by content, see compress_image_bytes)."""
    processed_photos = []
    if uploaded_files:
        for file in uploaded_files:
            try:
                file_bytes = file.getvalue()
                compressed_bytes = compress_image_bytes(file_bytes)
...
def main():
    ...
    with tab_report:
        ...
        total_photos = sum(len(v) for v in ss.photo_data.values())
        ...
        if st.button("📄 Build PDF Report", type="primary", use_container_width=True):
            ...
            if not all([inspector_name, inspector_phone, inspector_email, property_address, customer_name, customer_phone]):
                errors.append("Complete all required fields in sidebar (including customer phone)")
            if total_photos == 0:
                errors.append("Upload at least one photo")
            if hail_size == 0 and wind_speed == 0:
                errors.append("Enter hail size or wind speed")
            if wind_speed > 0 and not damage_notes.strip():
                errors.append("Please add damage description to support wind speed estimate")
