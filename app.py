...
def fetch_noaa_data(address: str, dol: str) -> Dict:
    ...
    In production, integrate with actual NOAA API or HailTrace alternative.
    
    Returns structured hail & wind radar data.
    """
    import random
    ...
    return {
        ...
        "storm_timestamp": datetime.now().strftime("%Y-%m-%d %H:%M UTC"),
    }


def generate_storm_risk_summary(noaa_data: Dict) -> str:
    """
    Generate AI/Consultant-style storm impact risk assessment based on NOAA data.
    """
    ...
    summary = (
        f"<b>Storm Impact Risk Assessment: {risk_level}</b><br/>"
        ...
        f"{noaa_data['distance_from_property_miles']} miles from property center. "
        f"Direction: {noaa_data['storm_direction']}. "
        f"Recommend detailed inspection of all elevated surfaces, vent penetrations, and soft-metal components."
    )
    return summary
...
def generate_adjuster_pdf(
    ...
    Args:
        inspector_name, inspector_phone, inspector_email: Field rep details
        property_address, customer_name, dol: Property metadata
        local_office: Service area (e.g., "St. Louis, MO")
        noaa_data: Storm radar data from NOAA simulation
        photo_categories_data: Dict of {category_name: [photo_list]}
        logo_path: Path to Belmont Construction logo (optional)
    ...
    # Corporate Header Table (2-column)
    left_header_data = [
        [COMPANY_NAME],
        [f"HQ: {COMPANY_HQ}"],
        [f"Service Area: {local_office}"],
    ]
    
    right_header_data = [
        ["PREPARED BY:"],
        [inspector_name],
        [f"Phone: {inspector_phone}"],
        [f"Email: {inspector_email}"],
    ]
    
    header_table_data = [
        [
            Table(left_header_data, colWidths=[3*inch]),
            Table(right_header_data, colWidths=[3*inch])
        ]
    ]
    ...
    noaa_data_table_data = [
        ...
        ["Distance from Property", f"{noaa_data['distance_from_property_miles']} miles"],
    ...
    risk_summary = generate_storm_risk_summary(noaa_data)
    ...
    for category_name, photo_list in photo_categories_data.items():
        ...
        # Create 3-column photo grid (3 photos per row, up to 4 rows = 12 per page)
        for i in range(0, len(photo_list), 3):
            row_photos = photo_list[i:i+3]
            
            # Build row data with photos and captions
            row_data = []
            for photo_dict in row_photos:
                try:
                    # Create image object from compressed bytes
                    img_bytes = io.BytesIO(photo_dict['compressed_bytes'])
                    img = RLImage(img_bytes, width=1.8*inch, height=1.35*inch)
                    
                    # Caption below photo
                    caption_text = photo_dict['filename'].split('.')[0][:30]
                    caption = Paragraph(f"<font size=7>{caption_text}</font>", NORMAL_STYLE)
                    
                    # Cell content: image + caption
                    cell_content = Table(
                        [[img], [caption]],
                        colWidths=[1.8*inch],
                        rowHeights=[1.35*inch, 0.4*inch]
                    )
...
def main():
    ...
    with st.sidebar:
        ...
        inspector_name = st.text_input("Inspector Name", placeholder="Your Full Name")
            ...
            if uploaded_files:
                ...
                for idx, photo in enumerate(processed[:3]):
                    with preview_cols[idx % len(preview_cols)]:
                        st.image(photo['compressed_bytes'], caption=photo['filename'][:20], width=120)
    ...
    if fetch_noaa:
        ...
        with col3:
            st.metric("Distance from Property", f"{noaa_data['distance_from_property_miles']} mi")
    else:
        noaa_data = None
